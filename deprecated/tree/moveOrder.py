...
# ----------------------------------------------------------------
#               MoveOrder Optimized Logic with PST
# ----------------------------------------------------------------
...
def alphabeta_ordered(board, depth, ply, alpha, beta, maximizing_player, positions_searched, null_move_made=False):
    ...
    is_not_endgame = any(board.pieces(piece_type, board.turn) for piece_type in [chess.ROOK, chess.QUEEN])
    
    # Add the "and not null_move_made" check
    if depth >= 3 and not board.is_check() and is_not_endgame and not null_move_made:
        # Make a null move
        board.push(chess.Move.null())
        
        # Search with reduced depth (R=2 is common) and set null_move_made=True
        score = -alphabeta_ordered(board, depth - 1 - 2, ply + 1, -beta, -beta + 1, not maximizing_player, positions_searched, null_move_made=True)
        
        board.pop() # Undo the null move
        
        # If the null move search fails high, we can prune this whole branch
        if score >= beta:
            # You can add a condition here to not return beta if the score is a mate score
            # to avoid illegal mate scores, but for now this is fine.
            return beta
    ...
    for depth in range(1, max_depth + 1):
        ...
        if board.turn == chess.WHITE:
            ...
            for move in legal_moves:
                board.push(move)
                board_value = alphabeta_ordered(board, depth - 1, 1, -math.inf, math.inf, False, positions_searched, null_move_made=False)
                board.pop()
            ...
            for move in legal_moves:
                board.push(move)
                board_value = alphabeta_ordered(board, depth - 1, 1, -math.inf, math.inf, True, positions_searched, null_move_made=False)
                board.pop()
